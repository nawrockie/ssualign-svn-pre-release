...
for alignments to models created using \prog{ssu-build}.  To draw
alignment statistic diagrams of our \prog{myseqs} example dataset, do:
...
\prog{myseqs.archaea.ps} file \footnote{For Mac users: the standard
...
positions on the diagrams if \prog{ssu-mask} has already been executed
...
(e.g. \prog{ssu-mask --key-out df -d myseqs}) as shown previously in
the masking section.
