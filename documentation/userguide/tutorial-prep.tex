...
or multi-threading. However, simplistic parallelization is possible
with the \prog{ssu-prep} program, if you have access to a compute
cluster or multi-core computers.
...
from the 3 default models of \textsc{ssu-align}.
...
By default, if \prog{-x} nor \prog{-y} options are used, the program
...
cluster. The relevant \prog{prefix/suffix} file for our specific
computing environment is included in
\prog{tutorial/janelia-cluster-presuf.txt}. 
...
the sequence file and partitioned it into 5 new files and placed them
in the newly created \prog{my30} subdirectory. Each of these files has
...
using \prog{qsub}. Note that the \prog{qsub} command line is composed
...
your system, replace the prefix and suffix strings in
\prog{janelia-cluster-presuf.txt} with strings specific to your
system. 
...
\prog{sh my30.ssu-align.sh \footnote{If this command fails, you're
...
\subsubsection{Using \prog{ssu-prep} to parallelize \prog{ssu-align}
