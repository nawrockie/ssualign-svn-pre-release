...
As shown in the tutorial, the \prog{ssu-align} command above prints
...
which are created in \prog{myseqs/}, a subdirectory of the directory
where the \prog{ssu-align} command was executed.
...
listed in \prog{myseqs/myseqs.archaea.hitlist}. These sequences were
...
\prog{<x>} bits with the \prog{-t <x>} option to \prog{ssu-align}. All
...
described in section~\ref{sec:models}. A \prog{0} at position \prog{x}
indicates that consensus position \prog{x} is excluded from the mask
and will be removed from the alignment when the mask is applied. A
\prog{1} at position \prog{x} indicates that it is included by the
...
mask. Black positions are kept by the mask. Either \prog{.pdf} or
...
the same length. In \prog{.afa} files created by \prog{ssu-mask},
