...
misaligned with respect to each other. This is because \sft{ssu-align}
...
profile-based alignment (at least as implemented in \sft{infernal} and
\sft{ssu-align}). In contrast, a nearest-neighbor based method might
...
phylogenetic inference, inserted columns in \sft{ssu-align} alignments
...
\sft{ssu-align}'s variability in \emph{insert} column number contrasts
markedly with the popular nearest-neighbor based aligners \sft{nast}
and \sft{sina}, which generate alignments of a fixed with (7,682
columns for \sft{nast} and 50,000 for \sft{sina}). However to
guarantee a fixed width, \sft{nast} must deliberately introduce errors
in the alignment in some cases \cite{DeSantis06}. 

\sft{ssu-align} generates alignments with a fixed number of consensus
columns. The philosophy is that the consensus columns are the
most important ones (e.g. for phylogenetic inference) and inserts are less
important. But by not restricting the number of inserts after each
consensus position, \sft{ssu-align} doesn't need to introduce
misalignments like \sft{nast}. 

HERE HERE HERE 
%with the trade-off being\ that the
%(hopefully) rare inserted nucleotides are not aligned.
