...
on how \sft{ssu-align} aligns SSU sequences, the distinction between
...
examples of using the programs in \sft{ssu-align} to create
...
derived from \sft{crw}. Section~\ref{sec:masks} details how the
...
format of output files generated by \sft{ssu-align} programs.
...
The six \sft{ssu-align} programs: \prog{ssu-align}, \prog{ssu-build},
\prog{ssu-draw}, \prog{ssu-mask}, \prog{ssu-merge} and \prog{ssu-prep}
are \sft{perl} scripts.  The package includes the three default (archaea,
...
\sft{ssu-align} bundles the \sft{infernal} software package, which is
written in C and is installed along with \sft{ssu-align} by following
the directions in the Installation section of this
guide. \sft{infernal} itself includes Sean Eddy's \sft{hmmer} package
and his sequence analysis library \sft{easel}.
...
compiler and \software{perl} (Practical Extraction and Report
...
\sft{ssu-align} does not yet have an associated publication,
so please cite the \sft{infernal} software publication
