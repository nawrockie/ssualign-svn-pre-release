...
required before you can use \sft{ssu-align}, as explained immediately
...
\prog{ssu-align} \sft{perl} script, the \prog{sa-0p1.params}
parameters file, and other important files as described in ``SSU-align
files'' below.  The \sft{infernal} user's guide is in the top-level
\prog{infernal} directory. The man pages are in the
...
\prog{make install}, can be found in the \sft{infernal} user's guide
which is in  PDF format in the top-level \prog{infernal/} directory. 

\emph{IMPORTANT:}
At this point, make sure the \sft{infernal}
programs (\prog{cmalign},  \prog{cmbuild}, \prog{cmcalibrate},
\prog{cmemit}, \prog{cmscore}, \prog{cmsearch}, and \prog{cmstat}) in
the \prog{src/} subdirectory as well as the \sft{easel} programs in
the \prog{easel/miniapps/} subdirectory are in your \$PATH\@.
You'll also want to make sure that the \prog{ssu-align} PERL script is
in your \$PATH\@. Once it is, typing \prog{ssu-align} will run it
if \sft{perl} is in \prog{/usr/bin/perl}. If \sft{perl} is somewhere
else (you can determine where it is with \prog{which perl}), you can
either change the first line of the \sft{ssu-align} script to include
the actual location instead of \prog{/usr/bin/perl}, or use \\
\prog{perl ssu-align} whenever running the script. The Tutorial section
includes several examples of running \prog{ssu-align} on small datasets.

\subsection{An additional crucial step that is required before you
can run SSU-align}

The \prog{ssu-align} \sft{perl} script needs to know where the
...
\prog{sa-userguide.pdf}, and the parameters file 
\prog{sa-0p1.params}. 
%You will need to manually edit the parameters file as
%explained below. 
...
\item[\emprog{seeds}] this includes the Stockholm formatted alignments
  ...
  created by the \prog{esl-ssudraw} program as explained in the Tutorial.

\item[\emprog{tutorial}] this includes some files that are used in the
  Tutorial section.

\item[\emprog{scripts}] this includes four simple \sft{perl}
