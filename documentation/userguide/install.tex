...
with 'ssu-cm', e.g. \prog{ssu-cmsearch}, and EASEL programs
called \emph{miniapps}, these are all named with a 'ssu-esl'
prefix, e.g. \prog{ssu-esl-seqstat}. The INFERNAL and
...
all programs are installed in \prog{/usr/local/man/man1}.  Finally,
data files and a PERL module (\prog{ssu.pm} required by the programs
are placed in \prog{/usr/local/share/ssu-align-0.1}, by default. Data
...
If you are using the bash shell, do:

> source setup.ssu-align.bash

and then add the following three lines to the '.bashrc'
...
If you are using the C shell, do:

> source setup.ssu-align.csh

and then add the following three lines to the '.cshrc'
file in your home directory:
