...
The programs are now in the \prog{src/} subdirectory. The
\sft{infernal} user's guide in the \\ 
top-level \prog{infernal} directory. The man pages are in the
\prog{documentation/manpages}
...
\begin{sreoutput}
<[ssu-align-0.1]> cat sa-0p1.params 
...
It is vital that you replace these paths with absolute paths (that is,
no use of '~' or ``..'') to these executable files on your own
