...
Download the source tarball (\prog{ssualign.tar.gz}) from 
\htmladdnormallink{ftp://selab.janelia.org/pub/software/ssualign/}
                  {ftp://selab.janelia.org/pub/software/ssualign/}
or \\
\htmladdnormallink{http://ssualign.janelia.org}
                  {http://ssualign.janelia.org}

Unpack the software:

\user{tar xvf ssualign.tar.gz}

Go into the newly created top-level directory (named either
\prog{ssualign}, or \prog{ssualign-xx} where \prog{xx} is a release
number:
...
\begin{enumerate}
\item
Edit the top of the Makefile. Set the paths and other make variables
to suit your system.

In partciular, you need to specify:
